...
class AL1Protocol:
    ...
    INTENTS = {
        "REQ": "Q", "RES": "R", "INF": "I", "ERR": "E", "LMB": "L", "ACT": "A", "ACK": "K"
    }
    ...
    def limb_i2c(self, bus, addr, reg, data, nonce=0):
        """Formats an I2C write as a MiniJSON LMB message."""
        payload = {"b": str(bus), "a": addr, "r": reg, "d": data}
        return self.encode("LMB", payload, nonce=nonce)

    def manifest(self, text, tags=None, nonce=0):
        """Formats a thought manifestation as a MiniJSON INF message."""
        payload = {"t": text}
        if tags:
            payload["g"] = tags
        return self.encode("INF", payload, nonce=nonce)

if __name__ == "__main__":
    loom = AL1Protocol("REGGIE")
